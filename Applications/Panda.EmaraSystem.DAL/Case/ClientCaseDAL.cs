...
namespace Panda.EmaraSystem.DAL
{
    public class ClientCaseDAL
    {
        ...
        public static ClientCase GetByClient(int clientId)
        {
            ClientCase clntCase = null;
            SqlConnection con;
            using (SqlDataReader dr = DataManager.GetDataReader("ESystem_ClientCaseGetByClient", out con,
                DataManager.CreateParameter("@ClientId", SqlDbType.Int, clientId)))
            {
